  ...
  return {
    ...
    provider: {
      ...
      async models(provider, ctx) {
        // internal boundary: SDK provider shim → internal Provider.Info
        const internal = provider as unknown as ProviderInfo
        if (ctx.auth?.type !== "oauth") {
          return Object.fromEntries(Object.entries(internal.models ?? {})) as unknown as Record<string, Model>
        }

        return Object.fromEntries(
          Object.entries(internal.models ?? {})
            .filter(([, model]) => {
              const api = (model as unknown as Model).api
              if (ALLOWED_MODELS.has(api.id)) return true
              const match = api.id.match(/^gpt-(\d+\.\d+)/)
            ...
            .map(([modelID, model]) => [
              modelID,
              {
                ...
                limit: String((model as { id: string }).id).includes("gpt-5.5")
                  ? {
                      context: 400_000,
                      input: 272_000,
                      output: 128_000,
