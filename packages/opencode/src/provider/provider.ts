...
export const ListResult = PublicOpencodeProviderListResultSchema
export type ListResult = Types.DeepMutable<Schema.Schema.Type<typeof ListResult>>

export const ConfigProvidersResult = PublicOpencodeConfigProvidersResultSchema
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const hook of plugins) {
          ...
          // internal boundary: SDK → internal model
          provider.models = yield* Effect.promise(async () => {
            const next = await models(toPublicInfo(provider), { auth: pluginAuth })
            const entries = Object.entries(next).map(([id, model]) => [
              id,
              {
                ...model,
                id: ModelV2.ID.make(id),
            ...
            return Object.fromEntries(entries) as unknown as Record<string, Model>
          })
        }
