...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const streamWithRuntime: Interface["streamWithRuntime"] = (input) =>
      Effect.gen(function* () {
        const scope = yield* Scope.make()
        const result = yield* Scope.provide(scope)(build(input))
        if (result.type === "native") {
          return {
            runtimeID: "native" as const,
            stream: result.stream.pipe(Stream.ensuring(Scope.close(scope, Exit.void))),
          }
        }
        const state = LLMAISDK.adapterState()
        return {
    ...
    return Service.of({ stream, streamWithRuntime })
  }),
)
