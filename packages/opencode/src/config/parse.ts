...
const JSONC_ERROR_LINE_LIMIT = 8
...
export function jsonc(text: string, filepath: string): unknown {
  ...
  if (errors.length) {
    const issues = errors
      .slice(0, JSONC_ERROR_LINE_LIMIT)
      .map((e) => {
        const beforeOffset = text.substring(0, e.offset).split("\n")
        const line = beforeOffset.length
        const column = beforeOffset[beforeOffset.length - 1].length + 1
        return `${printParseErrorCode(e.error)} at ${filepath}:${line}:${column}`
      })
      .join("\n")
    const more = errors.length > JSONC_ERROR_LINE_LIMIT ? `\n…and ${errors.length - JSONC_ERROR_LINE_LIMIT} more issue(s)` : ""
    throw new JsonError({
      path: filepath,
      message: issues + more,
    })
