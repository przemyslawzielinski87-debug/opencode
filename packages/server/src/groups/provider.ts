...
import { PublicProviderV2InfoSchema } from "@opencode-ai/core/public-provider"
...
export const ProviderGroup = HttpApiGroup.make("server.provider")
  .add(
    HttpApiEndpoint.get("provider.list", "/api/provider", {
      query: LocationQuery,
      success: Location.response(Schema.Array(PublicProviderV2InfoSchema)),
  ...
  .add(
    HttpApiEndpoint.get("provider.get", "/api/provider/:providerID", {
      ...
      query: LocationQuery,
      success: Location.response(PublicProviderV2InfoSchema),
